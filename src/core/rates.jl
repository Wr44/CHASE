mutable struct RatesCache
    ...
    spacer_loss :: Dict{Set{Int}, Float64}
...
function init_rates(
    ...
    rates = RatesCache(Dict(), Dict(), Dict(), Dict(), Dict(), Dict(), 0.0, sum(values(bacterias)))
    ...
    for (spacers, bacteria_count) in bacterias
        ...
        rates.spacer_loss[spacers] = spacer_loss_rate * length(spacers) * bacteria_count
...
function update_rates!(
    ...
    if event_type == :division || event_type == :death
        ...
        if n == 0
            ...
            old_sl  = get(cache.spacer_loss, spacers, 0.0)
            cache.lambda -= old_div + old_dth + old_sl
            delete!(cache.division, spacers)
            delete!(cache.death, spacers)
            delete!(cache.spacer_loss, spacers)
            ...
            new_sl  = spacer_loss_rate * length(spacers) * n
            cache.division[spacers] = new_div
            cache.death[spacers] = new_dth
            cache.spacer_loss[spacers] = new_sl
            cache.lambda += (new_div - old_div) + (new_dth - old_dth) + (new_sl - old_sl)
    ...
    elseif event_type == :infection_succeeded || event_type == :infection_failed
        ...
        update_rates!(cache, bacterias, phages, division_rate, death_rate, K,
                      phage_decay_rate, infection_rate, spacer_loss_rate, :division, spacers)
        update_rates!(cache, bacterias, phages, division_rate, death_rate, K,
                      phage_decay_rate, infection_rate, spacer_loss_rate, :phage_decay, phage_id)
    ...
    elseif event_type == :new_clone
        ...
        cache.death[spacers] = new_dth
        cache.spacer_loss[spacers] = new_sl
        cache.lambda += new_div + new_dth + new_sl
        for (phage_id, phage_count) in phages
            key = (spacers, phage_id)
            new_inf = infection_rate * n * phage_count
            if recognize(spacers, phage_id)
...
function sample_event(cache::RatesCache)
    ...
    for (k, v) in cache.spacer_loss
        acc += v
        if r < acc
            return (:spacer_loss, k)
        end
    end
